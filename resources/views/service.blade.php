@extends('layout.app')

@section('title', 'Сервис')
@section('description', 'Институт ЭКСТ осуществляет ремонт и техническое обслуживание оборудования на собственной производственной базе, а инженеры нашей компании предоставляют подробные консультации по обслуживанию и ремонту.')

@section('content')
    <main class="page">
        <section animate class="service-header">
            <picture>
                <source srcset="{{ asset('img/service-header.webp') }} 1x, {{ asset('img/service-header@2x.webp') }} 2x"
                        type="image/webp">
                <img srcset="{{ asset('img/service-header.jpg') }} 1x, {{ asset('img/service-header@2x.jpg') }} 2x"
                     src="{{ asset('img/service-header.jpg') }}" alt="Фоновое изображение"
                     class="service-header__background"/>
            </picture>
            <div class="service-header__container">
                <ul class="service-header__breadcrumb breadcrumb breadcrumb--light">
                    <li class="breadcrumb__item"><a href="{{ route('home') }}">Главная</a></li>
                    <li class="breadcrumb__item breadcrumb__item--active">Сервис</li>
                </ul>
                <h1 class="service-header__title">Сервис</h1>
                <div class="service-header__text">
                    <p>
                        Институт ЭКСТ осуществляет ремонт и техническое обслуживание оборудования на собственной
                        производственной базе, а инженеры нашей компании предоставляют подробные консультации по
                        обслуживанию и ремонту.
                    </p>
                    <p>
                        Помимо сервисного центра ЭКСТ, обслуживание и ремонт осуществляют региональные дилерские
                        центры, с перечнем которых более подробно можно ознакомиться
                        <a href="{{ route('contacts') }}">здесь</a>.
                    </p>
                </div>
            </div>
        </section>
        <section animate class="service-parts">
            <svg class="service-parts__decor" width="1750" height="1911" viewBox="0 0 1750 1911" fill="none"
                 xmlns="http://www.w3.org/2000/svg">
                <g opacity="0.03">
                    <path
                        d="M853.485 1695.21C853.485 1814.13 762.695 1910.51 650.72 1910.51C538.745 1910.51 447.954 1814.13 447.954 1695.21C447.954 1576.31 538.745 1479.91 650.72 1479.91C762.695 1479.91 853.485 1576.31 853.485 1695.21Z"
                        fill="#0077BD"/>
                    <path
                        d="M853.485 215.275C853.485 334.194 762.695 430.573 650.72 430.573C538.745 430.573 447.954 334.194 447.954 215.275C447.954 96.3792 538.745 0.000152922 650.72 0.000152922C762.695 0.000152922 853.485 96.3792 853.485 215.275Z"
                        fill="#373535"/>
                    <path
                        d="M1301.44 215.275C1301.44 334.194 1210.67 430.573 1098.69 430.573C986.695 430.573 895.926 334.194 895.926 215.275C895.926 96.3792 986.695 0.000152922 1098.69 0.000152922C1210.67 0.000152922 1301.44 96.3792 1301.44 215.275Z"
                        fill="#373535"/>
                    <path
                        d="M1749.4 215.275C1749.4 334.194 1658.63 430.573 1546.64 430.573C1434.66 430.573 1343.89 334.194 1343.89 215.275C1343.89 96.3792 1434.66 0.000152922 1546.64 0.000152922C1658.63 0.000152922 1749.4 96.3792 1749.4 215.275Z"
                        fill="#373535"/>
                    <path
                        d="M405.509 708.594C405.509 827.489 314.719 923.868 202.744 923.868C90.7689 923.868 0 827.489 0 708.594C0 589.698 90.7689 493.296 202.744 493.296C314.719 493.296 405.509 589.698 405.509 708.594Z"
                        fill="#0077BD"/>
                    <path
                        d="M853.485 708.594C853.485 827.489 762.695 923.868 650.72 923.868C538.745 923.868 447.954 827.489 447.954 708.594C447.954 589.698 538.745 493.296 650.72 493.296C762.695 493.296 853.485 589.698 853.485 708.594Z"
                        fill="#0077BD"/>
                    <path
                        d="M1301.44 708.594C1301.44 827.489 1210.67 923.868 1098.69 923.868C986.695 923.868 895.926 827.489 895.926 708.594C895.926 589.698 986.695 493.296 1098.69 493.296C1210.67 493.296 1301.44 589.698 1301.44 708.594Z"
                        fill="#0077BD"/>
                    <path
                        d="M853.485 1201.9C853.485 1320.82 762.695 1417.2 650.72 1417.2C538.745 1417.2 447.954 1320.82 447.954 1201.9C447.954 1083 538.745 986.624 650.72 986.624C762.695 986.624 853.485 1083 853.485 1201.9Z"
                        fill="#0077BD"/>
                    <path
                        d="M1301.44 1201.9C1301.44 1320.82 1210.67 1417.2 1098.69 1417.2C986.695 1417.2 895.926 1320.82 895.926 1201.9C895.926 1083 986.695 986.625 1098.69 986.625C1210.67 986.625 1301.44 1083 1301.44 1201.9Z"
                        fill="#373535"/>
                </g>
            </svg>
            <div class="service-parts__container">
                <h2 animate class="service-parts__title title-h2">
                    <svg class="title-h2__icon" width="49" height="50" viewBox="0 0 49 50" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M23.7169 44.3654C23.7169 47.4777 21.194 50 18.0823 50C14.9707 50 12.4478 47.4777 12.4478 44.3654C12.4478 41.2538 14.9707 38.7308 18.0823 38.7308C21.194 38.7308 23.7169 41.2538 23.7169 44.3654Z"
                            fill="#0077BD"/>
                        <path
                            d="M23.7169 5.63397C23.7169 8.74621 21.194 11.2686 18.0823 11.2686C14.9707 11.2686 12.4478 8.74621 12.4478 5.63397C12.4478 2.52234 14.9707 -3.8147e-06 18.0823 -3.8147e-06C21.194 -3.8147e-06 23.7169 2.52234 23.7169 5.63397Z"
                            fill="#373535"/>
                        <path
                            d="M36.165 5.63397C36.165 8.74621 33.6427 11.2686 30.5311 11.2686C27.4188 11.2686 24.8965 8.74621 24.8965 5.63397C24.8965 2.52234 27.4188 -3.8147e-06 30.5311 -3.8147e-06C33.6427 -3.8147e-06 36.165 2.52234 36.165 5.63397Z"
                            fill="#373535"/>
                        <path
                            d="M48.6133 5.63397C48.6133 8.74621 46.0909 11.2686 42.9787 11.2686C39.8671 11.2686 37.3447 8.74621 37.3447 5.63397C37.3447 2.52234 39.8671 -3.8147e-06 42.9787 -3.8147e-06C46.0909 -3.8147e-06 48.6133 2.52234 48.6133 5.63397Z"
                            fill="#373535"/>
                        <path
                            d="M11.2686 18.5447C11.2686 21.6564 8.74561 24.1787 5.63398 24.1787C2.52235 24.1787 0 21.6564 0 18.5447C0 15.4331 2.52235 12.9102 5.63398 12.9102C8.74561 12.9102 11.2686 15.4331 11.2686 18.5447Z"
                            fill="#0077BD"/>
                        <path
                            d="M23.7169 18.5447C23.7169 21.6564 21.194 24.1787 18.0823 24.1787C14.9707 24.1787 12.4478 21.6564 12.4478 18.5447C12.4478 15.4331 14.9707 12.9102 18.0823 12.9102C21.194 12.9102 23.7169 15.4331 23.7169 18.5447Z"
                            fill="#0077BD"/>
                        <path
                            d="M36.165 18.5447C36.165 21.6564 33.6427 24.1787 30.5311 24.1787C27.4188 24.1787 24.8965 21.6564 24.8965 18.5447C24.8965 15.4331 27.4188 12.9102 30.5311 12.9102C33.6427 12.9102 36.165 15.4331 36.165 18.5447Z"
                            fill="#0077BD"/>
                        <path
                            d="M23.7169 31.455C23.7169 34.5673 21.194 37.0896 18.0823 37.0896C14.9707 37.0896 12.4478 34.5673 12.4478 31.455C12.4478 28.3434 14.9707 25.821 18.0823 25.821C21.194 25.821 23.7169 28.3434 23.7169 31.455Z"
                            fill="#0077BD"/>
                        <path
                            d="M36.165 31.455C36.165 34.5673 33.6427 37.0896 30.5311 37.0896C27.4188 37.0896 24.8965 34.5673 24.8965 31.455C24.8965 28.3434 27.4188 25.821 30.5311 25.821C33.6427 25.821 36.165 28.3434 36.165 31.455Z"
                            fill="#373535"/>
                    </svg>
                    <div class="title-h2__content"><span>Запасные</span> <br/>части</div>
                </h2>
                <div class="service-parts__body service-body">
                    @foreach($parts as $part)
                        <div animate class="service-card">
                            <div class="service-card__image">
                                <picture>
                                    <source
                                        srcset="{{ $part->getFirstMedia('parts')->getUrl('partWebp') }} 1x, {{ $part->getFirstMedia('parts')->getUrl('partWebp@2') }} 2x"
                                        type="image/webp">
                                    <img src="{{ $part->getFirstMedia('parts')->getUrl('part') }}"
                                         srcset="{{ $part->getFirstMedia('parts')->getUrl('part') }} 1x, {{ $part->getFirstMedia('parts')->getUrl('part@2') }} 2x"
                                         alt="{{ $part->name }}"/></picture>
                            </div>
                            <div class="service-card__content">
                                <h2 class="service-card__title">{{ $part->name }}</h2>
                                <p class="service-card__description">
                                    {{ $part->description }}
                                </p>
                                <button data-popup="#popup" type="button" class="service-card__link link link--light">
                                    Заказать
                                </button>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </section>
        <section animate class="service-liquids">
            <div class="service-liquids__container">
                <div class="service-liquids__header">
                    <h2 animate class="service-liquids__title title-h2">
                        <svg class="title-h2__icon" width="49" height="50" viewBox="0 0 49 50" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M23.7169 44.3654C23.7169 47.4777 21.194 50 18.0823 50C14.9707 50 12.4478 47.4777 12.4478 44.3654C12.4478 41.2538 14.9707 38.7308 18.0823 38.7308C21.194 38.7308 23.7169 41.2538 23.7169 44.3654Z"
                                fill="#0077BD"/>
                            <path
                                d="M23.7169 5.63397C23.7169 8.74621 21.194 11.2686 18.0823 11.2686C14.9707 11.2686 12.4478 8.74621 12.4478 5.63397C12.4478 2.52234 14.9707 -3.8147e-06 18.0823 -3.8147e-06C21.194 -3.8147e-06 23.7169 2.52234 23.7169 5.63397Z"
                                fill="#373535"/>
                            <path
                                d="M36.165 5.63397C36.165 8.74621 33.6427 11.2686 30.5311 11.2686C27.4188 11.2686 24.8965 8.74621 24.8965 5.63397C24.8965 2.52234 27.4188 -3.8147e-06 30.5311 -3.8147e-06C33.6427 -3.8147e-06 36.165 2.52234 36.165 5.63397Z"
                                fill="#373535"/>
                            <path
                                d="M48.6133 5.63397C48.6133 8.74621 46.0909 11.2686 42.9787 11.2686C39.8671 11.2686 37.3447 8.74621 37.3447 5.63397C37.3447 2.52234 39.8671 -3.8147e-06 42.9787 -3.8147e-06C46.0909 -3.8147e-06 48.6133 2.52234 48.6133 5.63397Z"
                                fill="#373535"/>
                            <path
                                d="M11.2686 18.5447C11.2686 21.6564 8.74561 24.1787 5.63398 24.1787C2.52235 24.1787 0 21.6564 0 18.5447C0 15.4331 2.52235 12.9102 5.63398 12.9102C8.74561 12.9102 11.2686 15.4331 11.2686 18.5447Z"
                                fill="#0077BD"/>
                            <path
                                d="M23.7169 18.5447C23.7169 21.6564 21.194 24.1787 18.0823 24.1787C14.9707 24.1787 12.4478 21.6564 12.4478 18.5447C12.4478 15.4331 14.9707 12.9102 18.0823 12.9102C21.194 12.9102 23.7169 15.4331 23.7169 18.5447Z"
                                fill="#0077BD"/>
                            <path
                                d="M36.165 18.5447C36.165 21.6564 33.6427 24.1787 30.5311 24.1787C27.4188 24.1787 24.8965 21.6564 24.8965 18.5447C24.8965 15.4331 27.4188 12.9102 30.5311 12.9102C33.6427 12.9102 36.165 15.4331 36.165 18.5447Z"
                                fill="#0077BD"/>
                            <path
                                d="M23.7169 31.455C23.7169 34.5673 21.194 37.0896 18.0823 37.0896C14.9707 37.0896 12.4478 34.5673 12.4478 31.455C12.4478 28.3434 14.9707 25.821 18.0823 25.821C21.194 25.821 23.7169 28.3434 23.7169 31.455Z"
                                fill="#0077BD"/>
                            <path
                                d="M36.165 31.455C36.165 34.5673 33.6427 37.0896 30.5311 37.0896C27.4188 37.0896 24.8965 34.5673 24.8965 31.455C24.8965 28.3434 27.4188 25.821 30.5311 25.821C33.6427 25.821 36.165 28.3434 36.165 31.455Z"
                                fill="#373535"/>
                        </svg>
                        <div class="title-h2__content"><span>Чернила</span> <br/>и жидкости</div>
                    </h2>
                    <a href="{{ route('consumables') }}" class="service-liquids__link link link--light">
                        <span>Подробнее</span>
                        <svg width="7" height="13" viewBox="0 0 7 13" fill="none"
                             xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M6.53033 7.03033C6.82322 6.73744 6.82322 6.26256 6.53033 5.96967L1.75736 1.1967C1.46447 0.903806 0.989593 0.903806 0.696699 1.1967C0.403806 1.48959 0.403806 1.96447 0.696699 2.25736L4.93934 6.5L0.696699 10.7426C0.403806 11.0355 0.403806 11.5104 0.696699 11.8033C0.989593 12.0962 1.46447 12.0962 1.75736 11.8033L6.53033 7.03033ZM5 7.25H6V5.75H5V7.25Z"
                                fill="white"/>
                        </svg>
                    </a>
                </div>
                <div class="service-parts__body service-body">
                    @foreach($liquids as $liquid)
                        <div animate class="service-card">
                            <div class="service-card__image">
                                <picture>
                                    <source
                                        srcset="{{ $liquid->getFirstMedia('liquids')->getUrl('liquidWebp') }} 1x, {{ $liquid->getFirstMedia('liquids')->getUrl('liquidWebp@2') }} 2x"
                                        type="image/webp">
                                    <img src="{{ $liquid->getFirstMedia('liquids')->getUrl('liquid') }}"
                                         srcset="{{ $liquid->getFirstMedia('liquids')->getUrl('liquid') }} 1x, {{ $liquid->getFirstMedia('liquids')->getUrl('liquid@2') }} 2x"
                                         alt="{{ $liquid->name }}"/></picture>
                            </div>
                            <div class="service-card__content">
                                <h2 class="service-card__title">{{ $liquid->name }}</h2>
                                <p class="service-card__description">
                                    {{ $liquid->description }}
                                </p>
                                <button data-popup="#popup" type="button" class="service-card__link link link--light">
                                    Заказать
                                </button>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </section>
        <div class="service-caution">
            <div class="service-caution__container">
                <div animate class="service-caution__content">
                    В данном разделе представлены только основные запчасти и расходные материалы, для ознакомления с
                    полным перечнем продукции <a href="#" data-popup="#popup">обращайтесь к менеджерам</a>
                </div>
            </div>
        </div>
        <section animate class="service-software">
            <div class="service-software__container">
                <h2 class="service-software__title title-h2">
                    <svg class="title-h2__icon" width="49" height="50" viewBox="0 0 49 50" fill="none"
                         xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M23.7169 44.3654C23.7169 47.4777 21.194 50 18.0823 50C14.9707 50 12.4478 47.4777 12.4478 44.3654C12.4478 41.2538 14.9707 38.7308 18.0823 38.7308C21.194 38.7308 23.7169 41.2538 23.7169 44.3654Z"
                            fill="#0077BD"/>
                        <path
                            d="M23.7169 5.63397C23.7169 8.74621 21.194 11.2686 18.0823 11.2686C14.9707 11.2686 12.4478 8.74621 12.4478 5.63397C12.4478 2.52234 14.9707 -3.8147e-06 18.0823 -3.8147e-06C21.194 -3.8147e-06 23.7169 2.52234 23.7169 5.63397Z"
                            fill="#373535"/>
                        <path
                            d="M36.165 5.63397C36.165 8.74621 33.6427 11.2686 30.5311 11.2686C27.4188 11.2686 24.8965 8.74621 24.8965 5.63397C24.8965 2.52234 27.4188 -3.8147e-06 30.5311 -3.8147e-06C33.6427 -3.8147e-06 36.165 2.52234 36.165 5.63397Z"
                            fill="#373535"/>
                        <path
                            d="M48.6133 5.63397C48.6133 8.74621 46.0909 11.2686 42.9787 11.2686C39.8671 11.2686 37.3447 8.74621 37.3447 5.63397C37.3447 2.52234 39.8671 -3.8147e-06 42.9787 -3.8147e-06C46.0909 -3.8147e-06 48.6133 2.52234 48.6133 5.63397Z"
                            fill="#373535"/>
                        <path
                            d="M11.2686 18.5447C11.2686 21.6564 8.74561 24.1787 5.63398 24.1787C2.52235 24.1787 0 21.6564 0 18.5447C0 15.4331 2.52235 12.9102 5.63398 12.9102C8.74561 12.9102 11.2686 15.4331 11.2686 18.5447Z"
                            fill="#0077BD"/>
                        <path
                            d="M23.7169 18.5447C23.7169 21.6564 21.194 24.1787 18.0823 24.1787C14.9707 24.1787 12.4478 21.6564 12.4478 18.5447C12.4478 15.4331 14.9707 12.9102 18.0823 12.9102C21.194 12.9102 23.7169 15.4331 23.7169 18.5447Z"
                            fill="#0077BD"/>
                        <path
                            d="M36.165 18.5447C36.165 21.6564 33.6427 24.1787 30.5311 24.1787C27.4188 24.1787 24.8965 21.6564 24.8965 18.5447C24.8965 15.4331 27.4188 12.9102 30.5311 12.9102C33.6427 12.9102 36.165 15.4331 36.165 18.5447Z"
                            fill="#0077BD"/>
                        <path
                            d="M23.7169 31.455C23.7169 34.5673 21.194 37.0896 18.0823 37.0896C14.9707 37.0896 12.4478 34.5673 12.4478 31.455C12.4478 28.3434 14.9707 25.821 18.0823 25.821C21.194 25.821 23.7169 28.3434 23.7169 31.455Z"
                            fill="#0077BD"/>
                        <path
                            d="M36.165 31.455C36.165 34.5673 33.6427 37.0896 30.5311 37.0896C27.4188 37.0896 24.8965 34.5673 24.8965 31.455C24.8965 28.3434 27.4188 25.821 30.5311 25.821C33.6427 25.821 36.165 28.3434 36.165 31.455Z"
                            fill="#373535"/>
                    </svg>
                    <div class="title-h2__content">Программы</div>
                </h2>
                <div class="service-parts__body service-body">
                    @foreach($programs as $program)
                        <div animate class="service-card">
                            <div class="service-card__image">
                                <picture>
                                    <source
                                        srcset="{{ $program->getFirstMedia('programs')->getUrl('programWebp') }} 1x, {{ $program->getFirstMedia('programs')->getUrl('programWebp@2') }} 2x"
                                        type="image/webp">
                                    <img src="{{ $program->getFirstMedia('programs')->getUrl('program') }}"
                                         srcset="{{ $program->getFirstMedia('programs')->getUrl('program') }} 1x, {{ $program->getFirstMedia('programs')->getUrl('program@2') }} 2x"
                                         alt="{{ $program->name }}"/></picture>
                            </div>
                            <div class="service-card__content">
                                <h2 class="service-card__title">{{ $program->name }}</h2>
                                <p class="service-card__description">
                                    {{ $program->description }}
                                </p>
                                <a href="{{ route('service.show', $program->slug) }}"
                                   class="service-card__link link link--light">
                                    <span>Подробнее</span>
                                    <svg width="7" height="12" viewBox="0 0 7 12" fill="none"
                                         xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M6.53033 6.53033C6.82322 6.23744 6.82322 5.76256 6.53033 5.46967L1.75736 0.696699C1.46447 0.403806 0.989593 0.403806 0.696699 0.696699C0.403806 0.989593 0.403806 1.46447 0.696699 1.75736L4.93934 6L0.696699 10.2426C0.403806 10.5355 0.403806 11.0104 0.696699 11.3033C0.989593 11.5962 1.46447 11.5962 1.75736 11.3033L6.53033 6.53033ZM5 6.75H6V5.25H5V6.75Z"
                                            fill="#0077BD"/>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    @endforeach
                </div>
            </div>
        </section>
    </main>
@endsection
